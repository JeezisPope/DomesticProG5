...
//    picture: new FormControl()
  ...
  aceptar(): void {
    ...
    const existingUser = this.dataSource.data.find((e: Empleados) => e.email === email);
    if (existingUser?.email != null) {
      ...
      .onAction()
      .subscribe(() => this.router.navigateByUrl('/log-empleados'));
    } else {
      // Validar los campos
      if (
        name && name.length <= 50 &&
        ...
        this.router.navigate(['log-empleados']);
      } else {
        ...
        if (!age || age.toString().length > 3) {
          errorMessage += 'Edad (max 2 digitos)';
        ...
        if (!password || password.length > 50) {
          errorMessage += 'Contraseña (max 50 caracteres)   ';
        }
