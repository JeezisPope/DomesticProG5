...
export class EmpleadoresFormComponent implements OnInit{
  form: FormGroup = new FormGroup({});
  user: Empleadores = new Empleadores();
  mensaje: string = "";
  id: number = 0;
  dataSource = new MatTableDataSource<Empleadores>();
 ...
 const existingUser = this.dataSource.data.find((e: Empleadores) => e.email === email);
    if (existingUser?.email != null) {
      const errorMessage = `El email ya está en uso`;
      this.snackBar.open(errorMessage, 'Iniciar Sesión', {
        duration: 2000,
        horizontalPosition: this.horizontalPosition,
      })
      .onAction()
      .subscribe(() => this.router.navigateByUrl('/log-empleadores'));
    } else {
    if (
      name && name.length <= 20 &&
      dni && dni.length === 8 &&
